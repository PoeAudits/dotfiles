# Dotfiles Makefile
.DEFAULT_GOAL := help
.PHONY: help apply diff status update edit env-gen env-check server-status server-restart server-logs ufw-lockdown ufw-status
...
ufw-lockdown: ## Lock UFW to Tailscale-only inbound
	ufw-tailscale-lockdown

ufw-status: ## Show UFW status (verbose)
